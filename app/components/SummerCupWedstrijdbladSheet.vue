<script setup lang="ts">
import { framesPerMatch } from "#shared/summerCup/wedstrijdblad";

const props = defineProps<{
  /** Number of player rows / matrix size (4–8). */
  count: number;
  /** Player names by row; missing or empty entries render as blank cells. */
  playerNames?: string[];
}>();

// 4-5 players play 2 frames per match, 6-8 players play 1.
const frames = computed(() => framesPerMatch(props.count));

function printSheet() {
  window.print();
}
</script>

<template>
  <div class="wedstrijdblad min-h-screen bg-neutral-200 py-8 text-black">
    <!-- Screen-only toolbar. The slot owns its inner layout; the default is a
         hint plus an Afdrukken button. The `print` slot prop lets custom
         controls trigger printing too. -->
    <div class="mx-auto mb-6 max-w-[277mm] px-6 print:hidden">
      <slot name="controls" :print="printSheet">
        <div class="flex items-center justify-between">
          <span class="text-sm text-neutral-600">Afdrukken als liggend A4.</span>
          <button
            type="button"
            class="rounded bg-neutral-800 px-4 py-2 text-sm font-semibold text-white hover:bg-neutral-700"
            @click="printSheet"
          >
            Afdrukken
          </button>
        </div>
      </slot>
    </div>

    <!-- The printable sheet -->
    <div class="sheet mx-auto max-w-[277mm] bg-white p-10 shadow-lg print:max-w-none print:p-0 print:shadow-none">
      <header class="mb-6 flex flex-col gap-3 border-b-2 border-black pb-3">
        <img src="~/assets/images/Play-ER.svg" alt="Play-ER logo" class="h-12 w-auto self-start" />
        <slot name="header" />
      </header>

      <p v-if="count === 0" class="text-lg">Nog geen inschrijvingen.</p>

      <div v-else class="flex flex-wrap items-start gap-6">
        <!-- Player legend with a handicap start-score column.
             Cells are h-10 to match the matrix rows so the lines line up. -->
        <table class="border-collapse">
          <caption class="caption-bottom pt-2 text-sm font-medium text-neutral-700">
            Ere &amp; 1ste: 0, 2de &amp; 3de: 5, 4de &amp; 5de: 10
          </caption>
          <thead>
            <tr>
              <th class="h-10 border border-black px-2 font-semibold">#</th>
              <th class="h-10 min-w-50 border border-black px-4 text-left font-semibold">Speler</th>
              <th class="h-10 border border-black px-3 font-semibold">Startscore</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="i in count" :key="i">
              <th class="h-10 border border-black px-2 text-center font-semibold">{{ i }}</th>
              <td class="h-10 min-w-50 border border-black px-4 whitespace-nowrap">{{ playerNames?.[i - 1] ?? "" }}</td>
              <td class="h-10 border border-black px-3"/>
            </tr>
          </tbody>
        </table>

        <!-- Blank round-robin results matrix -->
        <table class="border-collapse text-center">
          <caption class="caption-bottom pt-2 text-sm font-medium text-neutral-700">
            Telkens {{ frames }} {{ frames === 1 ? "frame" : "frames" }}
          </caption>
          <thead>
            <tr>
              <th class="h-10 w-10 border border-black font-semibold">#</th>
              <th
                v-for="j in count"
                :key="j"
                class="h-10 w-10 border border-black font-semibold"
              >
                {{ j }}
              </th>
              <th class="h-10 border border-black px-3 font-semibold">Totaal</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="i in count" :key="i">
              <th class="h-10 w-10 border border-black font-semibold">{{ i }}</th>
              <td
                v-for="j in count"
                :key="j"
                class="h-10 w-10 border border-black"
                :class="i === j ? 'diagonal' : ''"
              />
              <td class="h-10 w-16 border border-black"/>
            </tr>
          </tbody>
        </table>

        <!-- Hand-fillable breaks (30+) beside the matrix; the first column is the
             player number, matching the legend, so the name is not repeated. The
             table grows to fill the row, with a 4 cm (40mm) minimum width. -->
        <table class="grow min-w-[40mm] border-collapse text-center">
          <thead>
            <tr>
              <th class="h-10 border border-black px-2 font-semibold">#</th>
              <th class="h-10 w-full border border-black px-3 text-left font-semibold">Breaks (30+)</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="i in count" :key="i">
              <th class="h-10 border border-black px-2 text-center font-semibold">{{ i }}</th>
              <td class="h-10 w-full border border-black px-3"/>
            </tr>
          </tbody>
        </table>
      </div>
    </div>
  </div>
</template>

<style scoped>
/* The blacked-out diagonal of the matrix; force it to print. */
.diagonal {
  background: black;
  print-color-adjust: exact;
  -webkit-print-color-adjust: exact;
}

@media print {
  @page {
    size: A4 landscape;
    margin: 12mm;
  }

  .wedstrijdblad {
    min-height: 0;
    padding: 0;
    background: white;
  }

  /* Make table borders render reliably in print. */
  .sheet {
    print-color-adjust: exact;
    -webkit-print-color-adjust: exact;
  }
}
</style>
